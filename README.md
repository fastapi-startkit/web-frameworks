...
plain text, `POST /user` → empty).

See [`fastapi-vs-fastapi-startkit.md`](fastapi-vs-fastapi-startkit.md) for the
full comparison (correctness, dependency setup, and a relative, same-host
performance delta) — updated after the load-test harness's
`--disable-keepalive` flag was removed.